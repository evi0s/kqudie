...
mongo.insert = async function (database, collection, insert_json) {
  ...
  try {
    ...
    if (!Array.isArray(insert_json))
      result = await collect.insertOne(insert_json);
    else
      result = await collect.insertMany(insert_json);
...
mongo.update = async function(database,collection,query_json,update_json,option = {}){
  ...
  try {
    let result = await db.collection(collection).update(query_json,update_json,{
      upsert, multi
    });
    connect.close();
    return result;
